...
def download_data():
  # lj_speech_dataset_path = kagglehub.dataset_download('mathurinache/the-lj-speech-dataset', path='/Users/evankaiden/Documents/SimpleTransfromerTTS-copy/content/speech_dataset', force_download=True)
  # ljspeech_meta_path = kagglehub.dataset_download('tttzof351/ljspeech-meta', path='/Users/evankaiden/Documents/SimpleTransfromerTTS-copy/content/speech_metadata', force_download=True)
  """
...
def batch_process(batch):
  ...
  text_padded = text_padded.to(hp.device)
  text_lengths = text_lengths.to(hp.device)
  mel_padded = mel_padded.to(hp.device)
  stop_token_padded = stop_token_padded.to(hp.device)
  mel_lengths = mel_lengths.to(hp.device)
...
def inference_utterance(model, text):
  sequences = text_to_seq(text).unsqueeze(0).to(hp.device)
...
if __name__ == "__main__":
  ...
  criterion = TTSLoss().to(hp.device)
  model = TransformerTTS().to(hp.device)
